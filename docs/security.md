...
| 11 | DoS via unbounded storage growth from `VerificationCount(BytesN<32>)` | The per-public-input-commitment counter added for analytics/abuse detection (`verification_count`, [#41](https://github.com/yusufadeagbo/zksoroban/issues/41)) is keyed by a commitment derived entirely from caller-supplied public inputs — the same attacker-controlled-key shape as finding #6. It reuses that finding's mitigation directly: entries live in `env.storage().temporary()` with `extend_ttl` refreshed on every write, so an unused commitment is evicted once its window elapses instead of accumulating forever. See `docs/architecture.md`'s Verification-Count Storage section. | **Pass** |
